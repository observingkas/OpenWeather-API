...
const deleteCityFromHistory = async (
  id: string
...
const renderForecastCard = (forecast: Weather) => {
  const { date, icon, description, temperature, humidity, windSpeed } =
    forecast;
...
const renderSearchHistory = async (searchHistory: Response): Promise<void> => {
  try {
    ...
    if (searchHistoryContainer) {
      searchHistoryContainer.innerHTML =
        '<p class="text-center">Error loading search history</p>';
    }
  }
};
...
const buildHistoryListItem = (city: City) => {
  console.log("Building history item for city:", city);
